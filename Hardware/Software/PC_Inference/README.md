...
### 1. Core Real-time Inference (核心实时推理)
...
* `model.py`: Definition of the **SimpleCNN architecture** (a 5-layer ConvNet optimized for embedded-to-PC inference).
...
* `yolo8_check.py`: Environment diagnostic tool to ensure CUDA/GPU acceleration is active for YOLOv8(although the experimenter only use CPU(intel i9)to train the model).
